...
n=1;
dist=-1000;
h = text(3,3,3,'No Collision!','FontSize',30);
cnt = 0;
for i = 2:-0.005:-2;
%     if n==60
%         break
%     end
    ...
    [collisionFlag] = GJK(S1Obj,S2Obj,iterationsAllowed);
    tic
    [dist,pts,G,H]=GJK_dist(S1Obj,S2Obj,iterationsAllowed);
    toc;
%     stock.pts{n}=pts;
    try delete(hhh)
    end
...
%     end
    ...
    if collisionFlag
        ...
        isEqual = strcmp(h.String, 'Collision!'); 
        h.String = 'No Collision!';
        if isEqual && cnt > 50
            break;
        end
    end
